...
HistNrPeopleWithGeom <- function(queues, rho, ...){
  ...
  geom_df = as.data.frame(lapply(1:nr_queues, function(i) rgeom(100000, rho[i])))
  colnames(geom_df) = 1:nr_queues
  geom_df = melt(geom_df)
  colnames(geom_df) = c('nr_queue', 'nr_people')

   plot = ggplot()+
     geom_histogram(data=queues_df, aes(x=nr_people, y=1*..density.., fill='Queue', weight=time_spent), alpha =0.8, binwidth=1, ...)+
     geom_histogram(data=geom_df, aes(x=nr_people, y=1*..density.., fill='Geometric', show_guide=FALSE), alpha = 0, colour='#FF9999',show_guide=FALSE, binwidth=1,  ...)+
     labs(x = "State (queue length)", y = "Proportion of time spent in state", size=16)+
